...
6.  En **ServidorWindows**, crea una GPO nueva denominada **Entorno Corporativo** y vincúlala a la OU de empleados. Evita usar **Default Domain Policy** para personalización de escritorio. Configura y comprueba los siguientes cambios:

    - configura un **papel tapiz corporativo** almacenado en un recurso interno del dominio (por ejemplo `\\ServidorWindows\NETLOGON\wallpaper.jpg`) con posición centrada
    - quita el menú ayuda del menú de Inicio
    - los usuarios no deben cambiar las configuraciones de escritorio (fondo, temas e iconos)
...
7.  Crea una nueva política de grupo que afecte a todos los objetos del dominio controlados por ServidorWindows denominada **“Normas Empresa”**. Esta nueva política tendrá las siguientes configuraciones:
    ...
    - oculta la pestaña de configuración del protector de pantalla para impedir cambios no autorizados
    - limitar el **idioma** de los usuarios a Español evitando que se puedan agregar nuevos
    - no queremos que los usuarios agreguen ninguna **impresora**. Tampoco queremos que eliminen ninguna impresora
    ...
    - configura la política de **actualizaciones automáticas** para que se instalen de forma planificada por TI (sin deshabilitarlas completamente)
...
10. Como has observado, la configuración de las políticas de grupo es un trabajo arduo. Realiza una copia de seguridad de las GPO en una ruta segura usando la opción de GPMC o un comando (`Backup-GPO` o `samba-tool gpo backup`) y programa esta copia periódica. Realiza esta configuración en **ServidorWindows**.
